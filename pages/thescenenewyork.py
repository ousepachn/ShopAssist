...
placeholder="What is a good restaurant for a first date?" 
##########################

# system message to 'prime' the model
primer = f"""You are restaurant reviewer bot.  A highly intelligent system that helps answer questions from users. 
...
st.info("""The demo is limited to the last 50 posts (no stories, highlights).\nHere are a few sample queries 
        [New speakeasy recommendations in new york?,   what are good brunch spots for big groups?,  intimate mexican dinner and cocktail recommendations?]""")  
...
if "messages6" not in st.session_state.keys():
    st.session_state["messages6"] = [{"role": "assistant", "content": "Ask about the products I featured to get link details and discount codes"}]
avatar = {
    "assistant": "🍴",
    "user": "🐱"
}
...
for message in st.session_state.messages6:
...
if prompt := st.chat_input(placeholder=placeholder):
    st.session_state.messages6.append({"role": "user", "content": prompt})
    ...
    st.session_state.messages6.append({"role": "assistant", "content": response})
